...
const createListItem = (knex, table, name) => knex
  .insert({
    name,
    ...genTimestamps(),
  })
...
exports.seed = (knex) => {
  ...
  const reactions = Promise.all([
    createListItem(knex, 'Reaction', 'Super'),
    createListItem(knex, 'Reaction', 'Okropieństwo'),
    createListItem(knex, 'Reaction', 'Obojętne'),
    createListItem(knex, 'Reaction', 'WTF'),
  ]);
      ...
      knex
        ...
        .then((articleIds) => {
          ...
          const reactionPromises = R.map(
            articleId => knex('ArticleReaction').insert([
              {
                articleId,
